...
//    [self.houseImageView sd_setImageWithURL:[NSURL URLWithString:attachment.messageDic[@"imageUrl"]]];
//    [self.houseImageView sd_setImageWithURL:[NSURL URLWithString:attachment.messageDic[@"imageUrl"]] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
//
//        NSLog(@"错误信息:%@",error);
//
//    }];
    [self.houseImageView sd_setImageWithURL:[NSURL URLWithString:attachment.messageDic[@"imageUrl"]] placeholderImage:[UIImage imageNamed:@"message_house_bj"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        
    }];
//    self.houseImageView.image = [UIImage imageNamed:@"broker_list_header"];
    //1租2售
    if (_type == 2) {

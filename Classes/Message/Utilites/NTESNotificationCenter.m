...
#import "TWSessionViewController.h"//消息页面
...
#import "NTESSessionMsgConverter.h"//设置自定义消息的类型
...
NIMBroadcastManagerDelegate, NIMSignalManagerDelegate,NIMChatManagerDelegate, NIMConversationManagerDelegate>
...
{
    UINavigationController *nav = [TWTabBarController instance].selectedViewController;
    BOOL needPlay = YES;
...
//    }
    for (UIViewController *vc in nav.viewControllers) {
        if ([vc isKindOfClass:[NIMSessionViewController class]] ||  [vc isKindOfClass:[TWSessionViewController class]] )
        {
            needPlay = NO;
            break;
        }
...
- (void)onRecvRevokeMessageNotification:(NIMRevokeMessageNotification *)notification
{
    ...
    TWTabBarController *tabVC = [TWTabBarController instance];
    UINavigationController *nav = tabVC.selectedViewController;

    for (TWSessionViewController *vc in nav.viewControllers) {
        if ([vc isKindOfClass:[TWSessionViewController class]]
...
{
    TWTabBarController *tab = [TWTabBarController instance];
    [tab.view endEditing:YES];
    if (tab.presentedViewController) {
        __weak TWTabBarController *wtabVC = tab;
...
{
    [[TWTabBarController instance].selectedViewController.view makeToast:content duration:2.0 position:CSToastPositionCenter];
}

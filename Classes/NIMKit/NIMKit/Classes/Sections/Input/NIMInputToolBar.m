...
@property(nonatomic,strong)UIImageView *twImageView;
...
//        self.tWCustomView.backgroundColor = [UIColor redColor];
        ...
        _twImageView.image = [UIImage imageNamed:@"message_phrase"];
        [self.tWCustomView addSubview:_twImageView];
        
        
        _twLabel = [[UILabel alloc]init];
        _twLabel.nim_size = CGSizeMake(43, 20);
        _twLabel.font = [UIFont systemFontOfSize:14];
        _twLabel.text = @"常用语";
        _twLabel.textAlignment = NSTextAlignmentLeft;
        [self.tWCustomView addSubview:_twLabel];
        
        _twCustomBtn = [[UIButton alloc]init];
        _twCustomBtn.backgroundColor = [UIColor clearColor];
        _twCustomBtn.selected = NO;
        _twCustomBtn.nim_size = CGSizeMake(71, _tWCustomView.nim_height);
        [self.tWCustomView addSubview:_twCustomBtn];
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    if (self.status == NIMInputStatusAudio) {
        ...
        //得到 ToolBar 自身高度
        viewHeight = viewHeight + 2 * self.spacing + 2 * self.textViewPadding+_tWCustomView.frame.size.height;
    }
...
- (void)layoutSubviews{
    ...
    for (NSNumber *type in self.types) {
        ...
        view.nim_centerY = (self.nim_height+_tWCustomView.frame.size.height) *0.5;
        left = view.nim_right;
    }
    ...
    _tWCustomView.nim_top = self.nim_top;
    
    _twImageView.nim_left = 10;
    _twImageView.nim_centerY= _tWCustomView.frame.size.height *0.5;
    _twLabel.nim_centerY =_tWCustomView.frame.size.height *0.5;
    _twLabel.nim_left = _twImageView.nim_right+10;
    _twCustomBtn.nim_left =_twImageView.nim_left;
    _twCustomBtn.nim_centerY = _tWCustomView.frame.size.height *0.5;
}

...
@interface NIMInputView()<NIMInputToolBarDelegate,NIMInputEmoticonProtocol,NIMContactSelectDelegate,TWPhraseViewDelegate>
{
...
@synthesize phraseContainer = _phraseContainer;
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    switch (self.status)
    {
        ...
        case NIMInputStatusMore:
            ...
            break;
        case NIMInputStatusPhrase:
            containerHeight = _phraseContainer.nim_height;
            break;
        default:
        {
...
- (void)refreshStatus:(NIMInputStatus)status
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.phraseContainer.hidden=status != NIMInputStatusPhrase;
    });
}
...
- (void)setup
{
    ...
    //添加常用语点击事件
    [_toolBar.twCustomBtn addTarget:self action:@selector(onTouchphraseBtn:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)checkPhraseContainer{
    if (!_phraseContainer) {
        TWPhraseView *phraseContainer = [[TWPhraseView alloc]initWithFrame:CGRectZero];
        phraseContainer.nim_size = [phraseContainer sizeThatFits:CGSizeMake(self.nim_width, CGFLOAT_MAX)];
        phraseContainer.delegate = self;
        phraseContainer.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        
        phraseContainer.hidden   = YES;
        _phraseContainer = phraseContainer;
    }
    
    if (!_phraseContainer.superview)
    {
        [self addSubview:_phraseContainer];
    }
}

-(void)setPhraseContainer:(UIView *)phraseContainer{
    _phraseContainer = phraseContainer;
    [self sizeToFit];
}
...
- (void)callDidChangeHeight
{
    ...
    {
        if (self.status == NIMInputStatusMore || self.status == NIMInputStatusEmoticon || self.status == NIMInputStatusAudio||self.status == NIMInputStatusPhrase)
        {
            //这个时候需要一个动画来模拟键盘
...
- (void)layoutSubviews
{
    ...
    _phraseContainer.nim_top = self.toolBar.nim_bottom;
}
...
- (void)onTouchEmoticonBtn:(id)sender
{
    if (self.status != NIMInputStatusEmoticon) {
        ...
        [self.phraseContainer setHidden:YES];
...
-(void)onTouchphraseBtn:(UIButton *)sender{
    sender.selected = !sender.selected;
    if (sender.selected) {
        if ([self.actionDelegate respondsToSelector:@selector(onPhraseBtn:)]) {
            [self.actionDelegate onPhraseBtn:sender];
        }
        
        [self checkPhraseContainer];
        [self bringSubviewToFront:self.phraseContainer];
        [self.phraseContainer setHidden:NO];
        [self.emoticonContainer setHidden:YES];
        [self.moreContainer setHidden:YES];
        [self refreshStatus:NIMInputStatusPhrase];
        [self sizeToFit];
...
#pragma mark --TWPhraseViewDelegate
//发送自定义消息
-(void)selectPhraseMessage:(NSString *)message{
    NSLog(@"---%@",message);
    NSString *sendText = message;
    [self.actionDelegate onSendText:sendText atUsers:[self.atCache allAtUid:sendText]];
    self.toolBar.twCustomBtn.selected = NO;
    [self.atCache clean];
    self.toolBar.showsKeyboard = YES;
}
